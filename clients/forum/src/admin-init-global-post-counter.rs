...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== MEMO-FORUM INITIALIZE GLOBAL COUNTER (ADMIN ONLY) ===");
    println!("This is a one-time setup operation to initialize the global post counter.");
    ...
    match client.get_account(&global_counter_pda) {
        Ok(account) => {
            println!("✅ Global counter already exists!");
            println!("   Account owner: {}", account.owner);
            println!("   Data length: {} bytes", account.data.len());
            
            // Verify it's owned by the correct program
            if account.owner == memo_forum_program_id {
                println!("   ✅ Owned by memo-forum program");
            } else {
            ...
            if account.data.len() >= 16 { // 8 bytes discriminator + 8 bytes u64
                ...
                println!("   Current total_posts: {}", total_posts);
            }
            
            println!();
            println!("No action needed. The global counter is already initialized.");
            return Ok(());
        },
        Err(_) => {
            println!("ℹ️  Global counter not found. Proceeding with initialization...");
        }
    }
    ...
    match client.send_and_confirm_transaction(&transaction) {
        Ok(signature) => {
            ...
            match client.get_account(&global_counter_pda) {
                Ok(account) => {
                    println!("✅ Global counter account created:");
                    ...
                    if account.data.len() >= 16 {
                        let total_posts_bytes = &account.data[8..16];
                        let total_posts = u64::from_le_bytes(total_posts_bytes.try_into().unwrap());
                        println!("   Initial total_posts: {}", total_posts);
                    }
                    
                    println!();
                    println!("✅ The memo-forum program is now ready to accept post creations!");
                    println!("   Next post created will have ID: 0");
                    println!("   Users can now create forum posts by burning tokens.");
                },
                Err(e) => {
        ...
        Err(err) => {
            ...
            if error_msg.contains("UnauthorizedAdmin") {
                ...
                println!("💡 Check that the memo-forum program is deployed and the program ID is correct.");
            } else {
