...
fn main() {
    ...
    let (single_max_burn_shard_pda, bump) = Pubkey::find_program_address(
        &[b"single_max_burn_shard"],
        &program_id
    );

    println!("Global Burn Index PDA: {}", global_burn_index_pda);
    println!("Single Max Burn Shard PDA: {}", single_max_burn_shard_pda);
    println!("Shard bump seed: {}", bump);

    // Double-check shard account status
    match client.get_account(&single_max_burn_shard_pda) {
        Ok(account) => {
            println!(
    ...
    let accounts = vec![
        ...
        AccountMeta::new(single_max_burn_shard_pda, false),
        AccountMeta::new_readonly(system_program::id(), false),
    ];

    // Prepare instruction data - Discriminator for 'initialize_single_max_burn_shard'
    // Note: You'll need to replace this with the actual discriminator from your compiled program
    let data = vec![212,52,230,22,91,4,146,118]; 
    ...
    println!("Sending transaction to initialize single max burn shard...");
    ...
    match client.send_and_confirm_transaction_with_spinner_and_config(
        ...
        Ok(signature) => {
            println!("Single max burn shard initialized successfully!");
            println!("Transaction signature: {}", signature);

            // Print account info
            println!("\nSingle Max Burn Shard Account Info:");
        ...
        Err(err) => {
            println!("Failed to initialize single max burn shard: {}", err);
            return;
        }
    }
