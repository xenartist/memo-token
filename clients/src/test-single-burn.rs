    ...
    match client.send_and_confirm_transaction(&transaction) {
        ...
        Err(err) => {
            ...
            match error_code {
                ...
                Some(6006) => println!("Error: UnauthorizedAdmin - Unauthorized: Only the admin can perform this action"),
                Some(6007) => println!("Error: BurnAmountTooSmall - Burn amount too small. Must burn at least 1 token."),
                Some(6008) => println!("Error: UnauthorizedUser - Unauthorized: Only the user can update their own profile"),
                Some(6009) => println!("Error: InvalidBurnAmount - Invalid burn amount. Must be an integer multiple of 1 token."),
                Some(6010) => println!("Error: InvalidBurnHistoryIndex - Invalid burn history index"),
                Some(6011) => println!("Error: BurnHistoryFull - Burn history account is full"),
                Some(6012) => println!("Error: CounterOverflow - Counter overflow: maximum number of shards reached"),
                Some(6013) => println!("Error: TopBurnShardFull - Top burn shard is full."),
                Some(6014) => println!("Error: InvalidTokenAccount - Invalid token account"),
