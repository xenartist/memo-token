...
fn main() {
    ...
    let (global_top_burn_index_pda, bump) = Pubkey::find_program_address(&[b"global_top_burn_index"], &program_id);
    ...
    match client.get_account(&global_top_burn_index_pda) {
        Ok(account) => {
            println!(
                "Account exists with {} bytes of data",
                account.data.len()
            );
        ...
        Err(err) => {
            ...
            println!("Nothing to close. Exiting.");
            return;
        }
    }
    ...
    // Close global top burn index instruction (discriminator from IDL)
    // Using the actual discriminator from IDL
    let data = vec![169, 205, 89, 205, 64, 5, 147, 219];
    ...
    println!("Sending transaction to close global top burn index...");
    ...
    match client.send_and_confirm_transaction_with_spinner_and_config(
        ...
        Ok(signature) => {
            println!("Global top burn index closed successfully!");
            println!("Transaction signature: {}", signature);

            // Print account info
            println!("\nClosed Global Top Burn Index Account Info:");
            println!("Program ID: {}", program_id);
            println!("Global Top Burn Index PDA: {}", global_top_burn_index_pda);
            println!("Your wallet (recipient): {}", payer.pubkey());
        ...
        Err(err) => {
            println!("Failed to close global top burn index: {}", err);
            println!("Note: If you're not the admin account, this operation will be rejected by the contract.");
            return;
        }
    }

    // Verify account is closed
    println!("Verifying account is closed...");
    let max_attempts = 5;
    let delay = Duration::from_millis(20000); // 20 seconds

    for attempt in 1..=max_attempts {
        match client.get_account(&global_top_burn_index_pda) {
            Ok(_) => {
                println!(
                    "Attempt {}/{}: Account still exists",
                    attempt, max_attempts
                );
                if attempt == max_attempts {
                    println!("Warning: Account still exists after {} attempts", max_attempts);
                } else {
                    sleep(delay);
                }
            }
