...
fn main() {
    ...
    let (single_max_burn_shard_pda, _bump) = Pubkey::find_program_address(
        &[b"single_max_burn_shard"],
        &program_id,
    );

    println!("Global Burn Index PDA: {}", global_burn_index_pda);
    println!("Single Max Burn Shard PDA to close: {}", single_max_burn_shard_pda);
    ...
    let accounts = vec![
        ...
        AccountMeta::new(single_max_burn_shard_pda, false),    // single_max_burn_shard account (writable)
        AccountMeta::new_readonly(system_program::id(), false), // system program
    ];

    // Prepare instruction data - Discriminator for 'close_single_max_burn_shard'
    // Note: You'll need to replace this with the actual discriminator from your compiled program
    let data = vec![72,62,227,2,35,25,22,135]; 
    ...
    println!("Sending transaction to close single max burn shard account...");

    match client.send_and_confirm_transaction(&transaction) {
        Ok(signature) => {
            println!("Single max burn shard account closed successfully!");
