...
fn main() {
    ...
    let space = 8 + // discriminator
                8 + // top_burn_shard_total_count (u64 needs 8 bytes)
                9;  // top_burn_shard_current_index (Option<u64>: 1 byte option tag + 8 bytes u64)
    ...
    // Initialize global top burn index instruction (discriminator from IDL)
    let data = vec![89, 23, 213, 27, 103, 194, 63, 67];
    ...
    println!("Sending transaction to initialize global top burn index...");
    ...
    match client.send_and_confirm_transaction_with_spinner_and_config(
        ...
        Ok(signature) => {
            println!("Global top burn index initialized successfully!");
            println!("Transaction signature: {}", signature);

            // Print account info
            println!("\nGlobal Top Burn Index Account Info:");
        ...
        Err(err) => {
            println!("Failed to initialize global top burn index: {}", err);
            return;
        }
    }
