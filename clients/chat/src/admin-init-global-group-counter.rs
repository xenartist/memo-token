...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== MEMO-CHAT INITIALIZE GLOBAL COUNTER (ADMIN ONLY) ===");
    println!("This is a one-time setup operation to initialize the global group counter.");
    ...
    match client.get_account(&global_counter_pda) {
        Ok(account) => {
            ...
            if account.data.len() >= 16 { // 8 bytes discriminator + 8 bytes u64
                ...
                println!("   Current total_groups: {}", total_groups);
            }
            ...
            match client.get_account(&global_counter_pda) {
                Ok(account) => {
                    ...
                    if account.data.len() >= 16 {
                        let total_groups_bytes = &account.data[8..16];
                        let total_groups = u64::from_le_bytes(total_groups_bytes.try_into().unwrap());
                        println!("   Initial total_groups: {}", total_groups);
                    }
                    
                    println!();
                    println!("✅ The memo-chat program is now ready to accept group creations!");
                    println!("   Next group created will have ID: 0");
                    println!("   Users can now create chat groups by burning tokens.");
                },
                Err(e) => {
